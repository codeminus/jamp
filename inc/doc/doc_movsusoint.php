...
$html .= '<table width="120%" align="center" style="font-family: Arial;font-size: 10pt">			
			...
			<tr>
				<td>
					Relat&oacute;rio de movimenta&ccedil;&otilde;es de sa&iacute;da por uso interno<br/>
					Gerado por: '.$_GET["tn"].'
				</td>
...
while($rs = mysql_fetch_object($sqlQuery)){
	...
	$html .= '<tr bgcolor="'.$color.'">
				<td align="center">'.$rs->mov_susoint_id.'</td>
				<td align="center">'.$rs->mov_susoint_prod_id.'</td>
				<td>'.$rs->classprod_nome.' - '.$rs->prod_nome.'</td>
				<td align="center" rowspan="4">'.date("d-m-y",$rs->mov_susoint_dt_saida).'</td>				
				<td align="center" rowspan="4">'.$rs->mov_susoint_quant_saida." ".$rs->prod_unid_medida.'</td>
				<td align="center" rowspan="4">'.number_format($rs->mov_susoint_svalor,"2",",",".").'</td>
				<td align="center" rowspan="4">'.number_format($rs->mov_susoint_svalor*$rs->mov_susoint_quant_saida,"2",",",".").'</td>
			  </tr>
			  <tr bgcolor="'.$color.'">
			  	<td>&nbsp;</td><td>&nbsp;</td>
			  ...
			  <tr bgcolor="'.$color.'">
			  	<td>&nbsp;</td><td>&nbsp;</td>
				<td><b>Solicitado por: </b>'.$rs->mov_susoint_solicitado_por.' <b>Autirizado por: </b>'.$rs->mov_susoint_autorizado_por.'</td>
			  </tr>
			  <tr bgcolor="'.$color.'">
			  	<td>&nbsp;</td><td>&nbsp;</td>
				<td><b>Finalidade: </b>'.$rs->mov_susoint_finalidade.'</td>				
			  </tr>';
...
$html .= '<tr bgcolor="'.$colors[0].'" style="color: '.$colors[1].'">
			<td colspan="7" align="center">'.$config["emp_site"].'</td>
		  </tr>		  
		  </table>		  
		  </body>';
